...
**M1.5(筆圧)実装済み・実機確認待ち。** [src/input.rs](../src/input.rs) の trait 抽象(PointerSource)にマウス(MouseSource)とペン(PenSource)を実装し、筆圧→半径/水量/顔料量のマッピングをスライダー化した。**octotablet は不採用**(下記 M1.5 行参照。Windows でメッセージループがデッドロックする既知バグ)。筆圧は egui 標準の Touch イベントで取る(winit が WM_POINTER から筆圧付きで届ける)。完了条件「ペンの筆入れ・筆抜きで水と顔料の量が変わり、ストロークに強弱が出る」は**実機のペンで目視確認したら完了にする**(UI 左パネル「筆圧 (M1.5)」に接地中の筆圧が出る)。確認できたら次は M2(乾燥とレイヤー)。エッジダークニング(FlowOutward)は M2 の乾燥と一緒に再検討(既定オフ。下記 M1d 行参照)。
