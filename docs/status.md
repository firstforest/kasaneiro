...
**M1.5(筆圧)実機確認済み・完了。次は M2(乾燥とレイヤー)。** [src/input.rs](../src/input.rs) の trait 抽象(PointerSource)にマウス(MouseSource)とペン(PenSource)を実装し、筆圧→半径/水量/顔料量のマッピングをスライダー化した。完了条件「ペンの筆入れ・筆抜きで水と顔料の量が変わり、ストロークに強弱が出る」を実機ペンで確認した。**octotablet は不採用**(下記 M1.5 行参照。Windows でメッセージループがデッドロックする既知バグ)。筆圧は egui 標準の Touch イベントで取る(winit が WM_POINTER から筆圧付きで届ける)。M2 は plan.md §3 参照: 湿レイヤー1枚+常時見える「乾かす」ボタン、乾燥=定着パスで RGBA レイヤーへ焼き込み、合成は multiply から、レイヤーパネル、Fast Dry / Wet the Layer。先送り中のエッジダークニング(FlowOutward)も M2 の乾燥と一緒に再検討(既定オフ。下記 M1d 行参照)。
