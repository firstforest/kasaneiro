...
**M3 進行中 — 削り2ツール(リフト/消去)+ 顔料個性(ρ/ω/γ)を目視確認済み。残りは KM 合成。** M3 を2スライスに分けた: ①削り+顔料個性(今回)②KM 合成(次回)。①はビルド・cargo test(13件)・clippy・実機起動(binding 9 レイアウト整合)に加え、**「塗る→リフトで色が浮いて縁へ流れ、ステイニング顔料は床が残る/バーントシェンナは紙目に溜まる」「消去で紙の白まで戻る」を目視確認済み**。②KM 合成は plan.md §3: レイヤー合成を multiply → KM の R/T 合成へ(km.rs の CPU 実装+テスト → WGSL 化。リニア色空間・sinh/cosh のオーバーフロー対策)、顔料 K,S を Curtis 12顔料表から導出してプリセット化。
