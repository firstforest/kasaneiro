...
**M1a〜M1d 目視確認済み・完了。次は M1.5(筆圧・octotablet)。** M1 の①水たまりの広がりと流れ(M1a)②色が流れて定着(M1b)③黄+青が緑に馴染む(M1c)④紙目ストリーク・粒状感(M1d)を目視確認した。**エッジダークニング(FlowOutward)だけは先送り**(既定オフ、M2 の乾燥と一緒に再検討。下記 M1d 行参照)。M1 の完了条件「先に置いた色の近くに別の色で描くと馴染んで綺麗なグラデーションになる」は達成。次は M1.5 で octotablet を入れ、筆圧→ブラシ半径/水量/顔料量へマッピングする(現状マウス直結の入力を [src/input.rs](../src/input.rs) の trait 抽象に載せ替える起点。plan.md §2 参照)。試行錯誤ループでのパラメータ調整は引き続きプリセット/ストローク再生で回す。
