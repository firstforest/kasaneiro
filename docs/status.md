...
**M2(乾燥とレイヤー)実装済み・目視確認待ち。** 「乾かす」= bake パス([assets/shaders/bake.wgsl](../assets/shaders/bake.wgsl))で湿レイヤーの顔料(浮遊+沈着)を乾燥レイヤーへ焼き込み、湿レイヤーを全ゼロに戻す。乾燥レイヤーは **RGBA 発色でなく4顔料濃度のまま** texture array(最大8枚)に保持し、表示時に白 latent と混ぜた「白地換算の透過率」を multiply 合成する(グレーズの重なりも mixbox を通る。M3 の KM 合成へのデータ互換も確保)。完了条件「乾かした色の上に別の色を描くと、下の層が動かず、重なりが透けて見える」の目視確認が次の作業。確認したら M2 を完了にして M3(削り・顔料個性・KM 合成)へ。
