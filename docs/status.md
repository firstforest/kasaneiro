...
**M2(乾燥とレイヤー)目視確認済み・完了。次は M3(削り・顔料個性・KM 合成)。** 完了条件「乾かした色の上に別の色を描くと、下の層が動かず、重なりが透けて見える」を目視確認した。エッジダークニング(dry_edge)は顔料被覆率ベースの縁バンドに直したが効きが弱いまま — 重要度が低いためこのままとする(強くしたくなったら「縁バンドへ内側から顔料を移す質量保存つき再分配」が次の一手。下記 M2 行参照)。M3 は plan.md §3 参照: リフティングツール(沈着層→浮遊層、ステイン床クランプ、紙目マスク、縁への再沈着)+完全消去ツールの2ツール構成、顔料パラメータ ρ/ω/γ + K,S のプリセット化(Curtis 12顔料)、レイヤー合成を multiply → KM の R/T 合成へ(km.rs の CPU 実装+テスト → WGSL 化)。
