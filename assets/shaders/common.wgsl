...
struct SimParams {
    ...
    dry_edge: f32,
    rewet_water: f32,
    tool: u32,           // M3: 0=描画 / 1=リフト(削り) / 2=消去 / 3=ぼかし筆(4=旧ならしは欠番)
    lift_strength: f32,  // M3: リフトの基準強度
    compose_mode: u32,   // M3: 0=multiply / 1=KM(R/T 光学合成)
    water_lift: f32,     // ぼかし筆(tool=3): 沈着顔料を浮遊層へ溶かし戻す率(馴染ませは拡散の物理が担う)
    ...
    _pad_paint: f32,        // 16B 整列の予備×3(次のパラメータ追加時はここから置き換える)
    _pad_paint2: f32,
    _pad_paint3: f32,
};
