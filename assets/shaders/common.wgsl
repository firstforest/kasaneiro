...
struct SimParams {
    ...
    paint_soak: f32,        // 置き馴染み: 既に濡れたセルへブラシ周囲まで水を持ち上げる係数(0=無効)
    paint_soak_radius: f32, // 置き馴染みの範囲(×ブラシ実効半径。1〜8 にクランプ)
    paint_pickup: f32,      // 色の溶かし戻し: 筆が触れた沈着顔料を浮遊層へ戻す割合(0=無効)
    paint_spread: f32,      // 広がる勢い: 置いた点から外向きの流れ(×筆圧×水量。0=無効)
    wet_hold: f32,          // 水持ち: 水が多いセルほど吸着を抑える(1に近いほど水中で沈まない)
    brush_charge: f32,      // 筆の含み(秒): 置いたまま色水が流れ出続ける時間(CPU 側で消費。0=無効)
    charge_pigment: f32,    // 含みから出る顔料の量(feed splat 1フレームの、通常 splat に対する比)
    diffuse_gamma: f32,     // 拡散の水依存カーブ γ: 流束の重み = 水量平均^γ(1=線形。大きいほど急峻)
};
