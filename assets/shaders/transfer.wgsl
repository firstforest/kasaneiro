...
// 吸着の水依存カーブ(note/07 で1本化): 水膜が保たれている間(w ≥ DEPOSIT_DRY_POINT)は
// DEPOSIT_WET_FLOOR に張り付き、膜が切れる(w がしきい値を下回る)と 2.0 へ立ち上がる。
// 実物の水彩で沈着が始まるのはウォッシュの流れが止まってから、をしきい値で表す。
// 旧カーブ((2−w)×(1−0.7w) 相当の単調減少)は、毛細管拡散(note/07)が水の山を薄く
// 延ばして運ぶ新モデルだと中間水量(0.3〜0.6)で吸着が強く、色が移動の途中で固まった。
// 再調整はホットリロード(H1)でここを直接編集
// 0.3 → 0.1(2026-07-09 目視2回目): 毛細管拡散は移流より運びが遅いぶん、顔料が長く
// 浮遊していないと広がる前に固まる。濡れている間の沈着は毎秒約14%(ρ=1)まで抑える
const DEPOSIT_WET_FLOOR: f32 = 0.1; // 水膜がある間の吸着倍率(0 にすると濡れている間は全く沈まない)
const DEPOSIT_DRY_POINT: f32 = 0.35; // 吸着が立ち上がる水量しきい値(上げるほど早く沈み始める)

fn deposit_weight(w: f32) -> f32 {
    return mix(2.0, DEPOSIT_WET_FLOOR, smoothstep(0.0, DEPOSIT_DRY_POINT, w));
}
