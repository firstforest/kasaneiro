...
impl PaintApp {
    ...
    pub(in crate::app) fn brush_panel(&mut self, ui: &mut egui::Ui) {
        ui.heading("水彩ブラシ");
        // 8ツールを1列(並列)に並べる: 「塗る」4色(色スウォッチ)+ 削り/消す/ぼかし筆/吸い取り
        // (文字ボタン)。**色選び = その色で「塗る」ツールを選ぶ**に統合したので、旧「ツール選択行 +
        // 別行の顔料スウォッチ」の2段はやめて全ボタンを一列にした(色スウォッチを押すと Paint ツール +
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            ui.add_space(8.0);
            // 削り/消す/ぼかし筆/吸い取り: 枠付きの文字ボタン(Paint はスウォッチ側で出すので飛ばす)。
        ...
        match self.tool.wet() {
            ...
            Some(WetTool::Absorb) => {
                ui.add(
                    egui::Slider::new(&mut self.params.absorb_rate, 0.0..=1.0)
                        .text("吸い取りの強さ"),
                )
                .on_hover_text(
                    "ひとなでで水と浮いている色を取り除く割合。強く押すほど多く吸う。\
                     紙に沈着した色は残る(剥がしたいときは「削り」)",
                );
            }
            _ => {}
        }
    ...
    pub(in crate::app) fn dried_info_panel(&mut self, ui: &mut egui::Ui, index: usize) {
        ...
        if ui
            ...
            .on_hover_text(
                "この層を描いたときのパレット(名前・性質ごと)へ丸ごと切り替えます。\n今の4色が惜しければ先に水彩レイヤーの「パレット(4色一式)」で保存してください",
            )
            .clicked()
        {
