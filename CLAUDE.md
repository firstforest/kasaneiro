...
**かさねいろ(kasaneiro)** — Rust + wgpu 製の水彩シミュレーションペイントツール(Windows 個人用ツール)。名前は「色を重ねる」+ 襲の色目から(リポジトリのディレクトリ名は歴史的経緯で my-paint のまま)。3つのコア要件 = **wet-on-wet**(色が馴染むグラデーション)/ **グレージング**(乾いた色の上に綺麗に重なる)/ **削り**(リフト+完全消去の2ツール)。ドキュメント・コミットメッセージは日本語。
