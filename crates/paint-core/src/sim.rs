...
pub struct SimParams {
    ...
    /// 広がり: 塗るブラシが既に濡れている(=描いてある)セルに置かれたときの、
    /// 筆の色が外へ広がる強さ。1つのノブで2つの仕組みを駆動する(splat.wgsl):
    /// ①水の足場 = ブラシ周囲(SOAK_RADIUS×実効半径)まで水を目標水位
    ///   brush_water×min(この値,1) へ持ち上げる(max 方式=なでても積み上がらない)
    /// ②外向きの流れ = 置いた点から放射流を与える(テクセル/ステップ。×筆圧×そのセルの水量)。
    /// 水がたっぷりの場所ほど遠く自由に流れて混ざる。乾いた紙のマスクは広げない
    /// (白紙へのストローク輪郭は従来どおり)。0=無効
    pub paint_spread: f32,
    /// 筆の含み(秒): 塗るブラシを置いたまま動かさなくても、この秒数だけ色水が
    /// 流れ出続ける(毎フレーム feed splat = 広がりの水補充+外向きの流れ+少量の顔料。
    /// 顔料量は brush_pigment×CHARGE_PIGMENT(splat.wgsl の定数))。
    /// 筆を下ろすたびに満たされ、置いている時間で減る。0=無効(従来どおり)
    pub brush_charge: f32,
    /// 16B 整列の予備(次のパラメータ追加時はここを置き換える)
    #[serde(skip)]
    pub _pad_paint: f32,
}
...
impl Default for SimParams {
    fn default() -> Self {
        Self {
            ...
            paint_spread: 0.8, // 広がり: 既存の塗りに筆を置くと水の足場+外向きの流れで色が広がる
...
pub struct Splat {
    ...
    /// 水・初速・顔料の一括注入(+=)はせず、広がりの水補充(max)+外向きの流れ+
