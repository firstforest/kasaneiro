...
pub struct SimParams {
    ...
    /// 置き馴染み: 塗るブラシ(tool=0)が、既に濡れている(=描いてある)セルに限り、
    /// ブラシ周囲(paint_soak_radius×実効半径)まで水を目標水位 brush_water×この係数 へ
    /// 持ち上げる強さ。既存の塗りに筆を置いたとき、色が外へ広がる「水の足場」を作る。
    /// max 方式なのでなでても積み上がらない。乾いた紙のマスクは広げない
    /// (白紙へのストローク輪郭は従来どおり)。0=無効
    pub paint_soak: f32,
    /// 置き馴染みの範囲: ブラシ実効半径に対する倍率(WGSL 側で 1〜4 にクランプ)
    pub paint_soak_radius: f32,
    ...
    /// 広がる勢い: 塗るブラシが既に濡れているセルに置かれたとき、置いた点から外向きの
    /// 流れを与える強さ(テクセル/ステップ)。実効値は筆圧と**そのセルの水量**でスケール
    /// するので、水がたっぷりの場所ほど遠く自由に流れて混ざる。0=無効
    pub paint_spread: f32,
    /// 水持ち: 水が多いセルほど吸着(沈着)を抑える度合い(transfer の吸着率に
    /// (1 − wet_hold×水量) を掛ける)。1 に近いほど、たっぷりの水の中では顔料が
    /// 沈まず浮遊し続けて自由に流れ・混ざる(沈着は水が引いてから)。0=従来どおり
    pub wet_hold: f32,
    /// 筆の含み(秒): 塗るブラシを置いたまま動かさなくても、この秒数だけ色水が
    /// 流れ出続ける(毎フレーム feed splat = 置き馴染みの水補充+広がる勢い+少量の顔料)。
    /// 筆を下ろすたびに満たされ、置いている時間で減る。0=無効(従来どおり)
    pub brush_charge: f32,
    /// 含みから出る顔料の量: feed splat 1フレームが注ぐ顔料の、通常 splat に対する比。
    /// 0 = 水だけが流れ出る(置いた色が水で運ばれて広がる)
    pub charge_pigment: f32,
    /// 拡散の水依存カーブ γ: 顔料拡散の流束の重み = (両セルの水量平均)^γ(diffuse.wgsl)。
    /// 1 = 線形(従来)。大きいほど「水がたっぷりのときは傾斜がなくても自由に混ざり、
    /// 水が引いてくると急に混ざらなくなる」が急峻になる
    pub diffuse_gamma: f32,
}
...
impl Default for SimParams {
    fn default() -> Self {
        Self {
            ...
            // 乾きかけで混ざらないのは diffuse_gamma の水依存カーブが担う
            pigment_diffuse: 0.4,
            diffuse_iters: 12,
            brush_channel: 0,
            pigment_density: 1.0,
            ...
            paint_soak: 0.8, // 置き馴染み: 既存の塗りに筆を置くと色が広がる(0 で従来どおり)
            paint_soak_radius: 3.0, // 広がる範囲はブラシ半径の3倍まで
            paint_pickup: 0.2, // 筆が下の色を軽く溶かして混ぜる(ω が高い顔料は残る)
            paint_spread: 0.8, // 置いた点から外向きの流れ(水が多い場所ほど遠くへ)
            wet_hold: 0.7, // 水がたっぷりの場所では顔料が沈まず流れ続ける
            brush_charge: 2.0, // 筆の含み: 置いたまま2秒は色水が流れ出続ける
            charge_pigment: 0.15, // 含みから出る顔料は控えめ(主に水が広げる)
            diffuse_gamma: 4.0, // 水たっぷり=自由に混ざる / 水が引く=混ざらない、を急峻に
        }
    }
}
...
pub struct Splat {
    ...
    /// 水・初速・顔料の一括注入(+=)はせず、置き馴染みの水補充(max)+広がる勢い+
    /// charge_pigment ぶんの顔料だけを注ぐ(splat.wgsl の描画ブランチが分岐)
    pub feed: f32,
    pub _pad: [f32; 2],
}
