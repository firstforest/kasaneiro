...
pub struct SimParams {
    ...
    /// ぼかし筆で描いた水路に顔料溜まりを接続したとき、色が水路へ広がる速さはここで稼ぐ
    ...
    pub rewet_water: f32,
    /// ブラシツール(M3/M4): 0=描画 / 1=リフト(削り) / 2=消去 / 3=ぼかし筆 / 4=吸い取り。
    /// splat.wgsl が分岐する。
    /// リフト = 沈着顔料を浮遊層へ戻して縁へ流す(ステイニング顔料は ω で残る)。
    /// 消去 = 湿レイヤーの水・顔料・濡れマスクを機械的にゼロへ(紙の白まで戻す完全消去)。
    /// ぼかし筆 = 水を置き、ブラシ下の沈着顔料を浮遊層へ溶かし戻す(water_lift)。馴染ませ自体は
    /// 毛細管拡散+γ重み顔料拡散の物理に任せる(2026-07-09 に箱ぼかしの均しを廃止して一次原理化)。
    /// 吸い取り = 乾いた筆で表面の自由水と浮遊顔料を取り除く(absorb_rate。
    /// 旧ならし tool=4 の跡地を 2026-07-10 に再利用)
    pub tool: u32,
    ...
    /// ぼかし筆(tool=3)の溶かし戻し率: ブラシ下の沈着顔料を浮遊層へ戻す割合
    /// (coverage×筆圧×(1−ω) で変調 = リフトの弱い版・paint_pickup と同じレシピ)。
    /// 浮いた顔料を馴染ませるのは毛細管拡散+γ重み顔料拡散の仕事(パラメータではなく物理)
    pub water_lift: f32,
    ...
    /// 色の溶かし戻し: 塗るブラシが触れた沈着顔料を浮遊層へ戻す割合(coverage×筆圧で変調。
    /// リフトの弱い版で、ステイニング ω の顔料は (1−ω) で残る)。筆の色と下の色が
    /// 同じ浮遊層で混ざるので、置いた境界が馴染む。0=無効
    ///
    /// なお「置いた色が外へ広がる」挙動はパラメータではなく水の毛細管拡散
    /// (diffuse.wgsl の WATER_DIFFUSE / WATER_GAMMA 定数、note/07)が担う:
    /// 広がる距離は置く水の量(brush_water)と含み(brush_charge)で決まる
    pub paint_pickup: f32,
    /// 筆の含み(秒): 塗るブラシを置いたまま動かさなくても、この秒数だけ色水が
    /// 流れ出続ける(毎フレーム feed splat = 目標水位への水補充(max)+少量の顔料
    /// (brush_pigment×CHARGE_PIGMENT、splat.wgsl の定数)。広げるのは毛細管拡散)。
    /// 筆を下ろすたびに満たされ、置いている時間で減る。0=無効(従来どおり)
    pub brush_charge: f32,
    /// 吸い取り(tool=4)の強さ: 1 splat で表面の自由水と浮遊顔料を取り除く割合
    /// (coverage×筆圧で変調)。浮遊顔料は水に乗っているだけなので水と同率で取れ、
    /// ω(染みつき)は関与しない。沈着顔料は紙に付いているので残る(剥がすのはリフト)
    pub absorb_rate: f32,
    /// 分離色: 2色目の顔料スロット(0..3)。brush_mix > 0 のとき、塗る筆が brush_channel と
    /// この2チャンネルへ顔料を按分して同時に注ぐ(splat.wgsl の描画・feed 両ブランチ)。
    /// 紙の上で2色が分かれるのはブラシの仕事ではなく顔料個性の差
    /// (粒の細かさ μ・密度 ρ・染みつき ω)が起こす物理
    pub brush_channel2: u32,
    /// 分離色の混合比: 0=単色(brush_channel のみ、従来どおり)/ 0.5=半々 / 1=2色目のみ。
    /// 総顔料量(brush_pigment)は変えず比率で按分する
    pub brush_mix: f32,
}
...
impl Default for SimParams {
    fn default() -> Self {
        Self {
            ...
            lift_rate: 0.1,
            // 水量 0.5 の塗りが約1分保つ値(0.0003×dt0.5×60fps ≈ 0.009/秒)。
            // 旧 0.005 は3〜4秒で乾いてしまい速すぎた(ユーザー調整 2026-07-11)。
            // 蒸発は実時間でなくステップ毎なので、モニタの Hz が上がると比例して速く乾く点に注意
            evap_rate: 0.0003,
            // 実効 0.1(0.2×dt0.5)= WGSL 安定上限 0.2 の半分。乾燥を約1分に遅くした際、
            // 混ざり続ける時間が15倍になって広がりすぎたため 0.4(上限張り付き)から半減した
            // (ユーザー調整 2026-07-11)。乾きかけで混ざらないのは水依存カーブ
            // (diffuse.wgsl の DIFFUSE_GAMMA)が担う。水の毛細管拡散(WATER_DIFFUSE)は
            // diffuse_iters 側に紐づくため影響しない=置いた色水の届く距離は不変
            pigment_diffuse: 0.2,
            diffuse_iters: 12,
            brush_channel: 0,
            pigment_density: 1.0,
            ...
            water_lift: 0.3, // ぼかし筆の溶かし戻し率。リフト(lift_strength 0.3)と同格の効き
            line_mode: 0,     // 既定は鉛筆
            line_eraser: 0,
            ...
            paint_pickup: 0.2, // 筆が下の色を軽く溶かして混ぜる(ω が高い顔料は残る)
            brush_charge: 2.0, // 筆の含み: 置いたまま2秒は色水が流れ出続ける
            absorb_rate: 0.5,  // 吸い取り: ひとなでで水たまりの大半を回収できる強さ
            brush_channel2: 3, // 分離色の2色目: 既定はバーントシェンナ(フタロと混ぜると分かれる対比)
            brush_mix: 0.0,    // 既定は単色(分離色はスライダーで混合比を上げて使う)
        }
    }
}
...
pub struct Splat {
    ...
    pub pressure: f32,
    /// 筆の含み(feed)フラグ: 1 = 置いたまま動かない間に毎フレーム注がれる色水。
    /// 水・初速・顔料の一括注入(+=)はせず、目標水位への水補充(max)+
    /// 少量の顔料(CHARGE_PIGMENT 比)だけを注ぐ(splat.wgsl の描画ブランチが分岐)
    pub feed: f32,
    pub _pad: [f32; 2],
}
...
impl Splat {
    ...
    /// 筆の含み(feed)splat: 置いたまま動かないフレームに1つ積む(初速なし)
    pub fn feed_at(pos: [f32; 2], pressure: f32) -> Self {
        Self {
            feed: 1.0,
            ..Self::new(pos, [0.0, 0.0], pressure)
        }
    }
}
